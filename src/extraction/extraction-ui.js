...
async function handlePDFUpload(file, statusElement) {
  showStatus(
    statusElement,
    "📖 Reading PDF... This may take a moment.",
    "loading",
  );
  ...
  try {
    ...
    showStatus(
      statusElement,
      "✨ Generating AI summary (analyzing " +
        pdfData.totalPages +
        " pages)...",
      "loading",
    );
...
function initializeYouTubeExtraction() {
  ...
  youtubeBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      showStatus(
        youtubeStatus,
        "⚠️ Could not auto-extract. Please use Transcript tab to paste manually.",
        "error",
      );
    } finally {
      youtubeBtn.disabled = false;
...
function initializeTranscriptExtraction() {
  ...
  transcriptBtn.addEventListener("click", async () => {
    ...
    if (text.length < 100) {
      showStatus(
        transcriptStatus,
        "⚠️ Text is too short. Please provide at least 100 characters.",
        "error",
      );
      return;
    }
...
function formatMarkdownContent(content) {
  let html = content
    .replace(
      /^### (.+)$/gm,
      '<h3 style="color: #6366f1; margin-top: 16px; margin-bottom: 8px;">$1</h3>',
    )
    .replace(
      /^## (.+)$/gm,
      '<h3 style="color: #6366f1; margin-top: 16px; margin-bottom: 8px; font-size: 16px;">$1</h3>',
    )
    .replace(
      /^# (.+)$/gm,
      '<h2 style="color: #6366f1; margin-top: 16px; margin-bottom: 8px; font-size: 18px;">$1</h2>',
    )
...
async function summarizePDF(pdfData) {
  ...
  const truncatedText =
    pdfData.fullText.length > maxChars
    ...
    if (typeof chrome !== "undefined" && chrome.scripting && chrome.tabs) {
      try {
        ...
        const createdTab = await new Promise((resolve) =>
          chrome.tabs.create({ url: videoPage, active: false }, resolve),
        );
                ...
                try {
                  ...
                  if (!collected.trim()) {
                    ...
                    if (transcriptPanel) {
                      ...
                      const drawerItems =
                        document.querySelectorAll("#body-text .cue");
                      drawerItems.forEach(
                        (it) => (collected += (it.textContent || "") + " "),
                      );
                    }
                  }
                ...
                try {
                  ...
                  selectors.forEach((sel) => {
                    document
                      ...
                      .forEach(
                        (node) => (collected += (node.textContent || "") + " "),
                      );
                  });
        ...
        if (pageTranscript && pageTranscript.length > 100)
...
async function callGeminiAPI(prompt) {
  ...
  if (!apiKey) {
    throw new Error(
      "Gemini API key is missing. Please add GEMINI_API_KEY to the .env file.",
    );
  }
