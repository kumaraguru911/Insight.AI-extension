...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === "showAIHistory") {
    ...
    chrome.scripting.insertCSS({
      target: { tabId: tab.id },
      files: ["src/styles/history.css"],
    });
    chrome.scripting.executeScript({
      target: { tabId: tab.id },
      files: ["src/content/history_content_script.js"],
    });
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "showAIResponseFromHistory") {
    ...
    chrome.storage.local.get([HISTORY_STORAGE_KEY], (result) => {
      sendResponse({ history: result[HISTORY_STORAGE_KEY] || [] });
    });
    return true; // Indicate that sendResponse will be called asynchronously
...
async function getGeminiResponse(text) {
  ...
  if (!apiKey) {
    console.error(
      "Gemini API key missing. Please add GEMINI_API_KEY to the .env file.",
    );
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const errorMessage =
        ...
        "An unknown API error occurred. Check the console for details.";
